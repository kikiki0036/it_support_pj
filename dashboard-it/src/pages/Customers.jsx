...
function EditProfileUser(props) {
    ...
    const { setMsg, IdUser, onCloseEditProfileUserDialog, openEditProfileUserDialog, ...other } = props;
        ...
        if(userProfile.length > 0) {
            ...
            try {
                await axios.post('http://localhost:5000/UpdateProfile', { 
                    ...
                    password: data.get('password-' + userProfile[0].id_user)
                ...
                userProfile.length > 0 ?
                    <>
                       ...
                       <FormControl sx={{width:"100%"}} variant="outlined">
                            ...
                            <OutlinedInput
                                id={"password-" + userProfile[0].id_user}
                                type={values.showPassword ? 'text' : 'password'}
                                defaultValue={userProfile[0].password}
                                name={"password-" + userProfile[0].id_user}
...
const Customers = () => {
    ...
    const [openEditProfileUserDialog, setOpenEditProfileUserDialog] = useState(false);
    const [openCreateProfileUserDialog, setOpenCreateProfileUserDialog] = useState(false);
    ...
    const DataUser = async (e) => {
        try {
            await axios.get('http://localhost:5000/getDataAllUser', { }).then((res) => {  
    ...
    const RenderBodyUser = (item, index) => {
        ...
        return (
            ...
            <tr key={index}  className={"cursor_pointer " + ( index % 2 !== 0 ? "boxc-b": "boxc-w")} >
        ...
        // popupState.close
                                    ...
                                    <Menu {...bindMenu(popupState)} className="MenuUser">
                                        ...
                                        <MenuItem sx={{display: "none"}}></MenuItem>
                                        {
                                            // UserProfile.length > 0 && UserProfile.findIndex(object => { return object.id_user === item.id_user }) !== -1 ?
                                                <MenuItem onClick={ () => {  popupState.close(); handleEditProfileUserDialog(item.id_user);  } } sx={{fontSize: 13}}>View Profile</MenuItem>
                                            //:
                ...
                <td className="col_txt">{item.gender}</td>
                <td className="col_txt">{item.level_user}</td>
