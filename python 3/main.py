...
def send_em():
    jsony = request.json
    sender = "kikiki034@hotmail.com"
    receivers = [jsony['receive']]
    SUBJECT = jsony['title']
    TEXT = jsony['msg']
    newm = email.message_from_string(TEXT)
    ...
    server.starttls()
    server.login("kikiki034@hotmail.com", "034854113za")
    server.sendmail(sender, receivers ,newm.as_string().encode('utf-8'))
    server.quit()
    return "ok"
...
def chatbot():
    ...
    if prob.item() > 0.86:
        ...
        return str("โปรดระบุปัญหาให้ชัดเจน")
    return str("ลองใหม่อีกครั้ง(คำที่พิมพ์มาใกล้เคียงกับในระบบ)")
