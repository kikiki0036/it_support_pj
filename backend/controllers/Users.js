...
export const getNotebook_center = async(req, res) => {
    ...
    try {
        const NotebookCenter = await Notebook_center.findAll({
            attributes: ['device_center_id','device_name']
        });
...
// export const RemoveNewServiceType = async(req, res) => {
...
//             attributes: ['id_option','title','status','whoAppr','id_type'],                  
...
//     const { id_option, title, whoAppr, status, id_type, objItems } = req.body;
...
//     const { id_option, title,  status, whoAppr, id_type, objItems, delItems } = req.body;

//     try {

//         await S_option.update({ title: title, status: status, whoAppr: whoAppr, id_type: id_type },{
//             where:{
//                 id_option : id_option ,
//             }
//         });
...
export const UpdateStatusTk = async(req, res) => {
    const { tikket_no, status, review_by } = req.body;
    try {

        await Tikket.update({ status: status,  review_by: review_by },{
            where:{
                tikket_no : tikket_no
            }
...
export const getDataAllUser = async(req, res) => {
    try {
        ...
        const AllUser = await UserAccount.findAll({
            ...
            attributes: ['id_user','name_user','name_user_eng','gender',`level_user`,`mail`,`password`,`role_id`]
...
export const UpdateProfile = async(req, res) => {
    const { id_user, name_user, name_user_eng, mail, password } = req.body;
   
    try {
        await UserAccount.update({ name_user: name_user, name_user_eng: name_user_eng, mail: mail, password: password }, {
            where: {
                id_user: id_user
            }
        });
...
export const Login = async(req, res) => {
    ...
    try {
        ...
        if( req.body.password != User_login[0].password ) return res.status(400).json({msg: "Wrong Password"});
       ...
       res.status(404).json({msg:"ID or Mail is incorrect"});
    }
}
